    ...
    for mt in MOVE_THRESHES:
        for vm in VOL_MULTS:
            for at in ANTITOP:
                tr = run(data, day_pos, ranks, mt, vm, at)
                m = metrics(tr, months)
                m.update({"move_thr": mt, "vol_mult": vm, "antitop": at})
                if tr:
                    m["rand_control_avg"] = random_control(data, min(len(tr), 400), seed=1)
                results.append(m)
                print("[bt] mt=%s vm=%s at=%s -> n=%s/mes=%s wr=%s exp=%s pf=%s ctrl=%s"
                      % (mt, vm, at, m.get("n"), m.get("per_month"), m.get("win_rate"),
                         m.get("expectancy"), m.get("profit_factor"), m.get("rand_control_avg")))
    out = {"generated": int(time.time()), "days": DAYS, "universe": len(data),
           "fee": FEE, "params": {"lookback": LOOKBACK, "atr_mult": ATR_MULT,
           "scaleout_r": SCALEOUT_R, "scaleout_frac": SCALEOUT_FRAC},
           "results": sorted(results, key=lambda x: (x.get("expectancy") or -99), reverse=True)}
    json.dump(out, open("/tmp/bt_results.json", "w"), indent=1)
    gh_put("docs/_btres.json", json.dumps(out, indent=1).encode(), "bt results %d" % out["generated"])
    print("[bt] LISTO en %.0fs" % (time.time() - t0))
