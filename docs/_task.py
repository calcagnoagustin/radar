...
info={"task":"a1_tail3","now":time.strftime("%Y-%m-%d %H:%M:%SZ",time.gmtime()),
      "log":sh("tail -25 %s/a1_retry.log"%B),
      "proceso":sh("pgrep -af a1_launcher.py"),
      "mem":sh("free -m | head -2"),"intentos":sh("grep -c intento %s/a1_retry.log"%B),
      "ultimo":sh("tail -1 %s/a1_retry.log"%B),"ok":True}
...
p={"message":"a1 tail3","content":base64.b64encode(json.dumps(info,indent=1,ensure_ascii=False).encode()).decode()}
if sha: p["sha"]=sha
